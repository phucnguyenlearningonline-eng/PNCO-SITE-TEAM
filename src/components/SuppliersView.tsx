...
export const SuppliersView: React.FC<SuppliersViewProps> = ({
  ...
  const handleDelete = (sup: Supplier) => {
    ...
    if (confirm(`Bạn có chắc chắn muốn xóa đối tác / nhà cung cấp "${sup.name}"?`)) {
          ...
          <div className="bg-white rounded-xl shadow-2xl border border-slate-200 w-full max-w-lg overflow-hidden animate-in fade-in zoom-in-95">
            <div className="bg-[#102742] text-white px-5 py-4 flex items-center justify-between">
              <h3 className="font-bold text-base">
                {editingSupplier ? 'Chỉnh Sửa Nhà Cung Cấp / Đối Tác' : 'Thêm Nhà Cung Cấp / Đội Xe / Quán Cơm'}
              </h3>
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-3 text-xs">
              <div>
                <label className="block font-bold text-slate-700 uppercase mb-1">
                  Tên Đơn Vị / Đối Tác <span className="text-rose-500">*</span>
                </label>
                <input
                  type="text"
                  value={name}
              ...
              <div className="flex justify-end gap-2 pt-3 border-t">
                ...
                >
                  {editingSupplier ? 'Lưu Cập Nhật' : 'Lưu Đơn Vị'}
                </button>
              </div>
            </form>
          </div>
