...
export const ClientsView: React.FC<ClientsViewProps> = ({
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    if (onEditProject) {
      ...
      onUpdateClient(selectedProject.client, clientName.trim());
    }
  ...
  const handleDelete = (p: Project) => {
    ...
    if (confirm(`Bạn có chắc chắn muốn xóa hồ sơ Khách hàng / Chủ đầu tư "${p.client}" của dự án "${p.name}"?`)) {
      ...
      <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-wrap items-center justify-between gap-3">
        <div>
          <h3 className="text-base font-bold text-slate-900 flex items-center gap-2">
            ...
            <span>DANH SÁCH CHỦ ĐẦU TƯ & KHÁCH HÀNG DỰ ÁN M&E ({projects.length})</span>
          </h3>
        ...
        {projects.map((p) => {
          ...
          return (
            ...
            >
              ...
              {/* Thông tin thực tế trong bản chi tiết: Số khoản chi phát sinh */}
              <div className="mt-4 pt-3 border-t border-slate-100 flex justify-between items-center text-xs">
                ...
                <span className="font-mono font-bold text-slate-900 text-xs bg-slate-100 px-2 py-0.5 rounded border border-slate-200">
                  {projectExpenses.length} giao dịch ({formatVND(totalSpent)})
                </span>
              </div>
            </div>
          );
